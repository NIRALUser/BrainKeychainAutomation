...
class BrainKeyCreatorExtensionWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    parametersFormLayout.addRow("Input surface folder (required, can not be named 'Keychains'):", self.inputDirSelector)
    ...
    parametersFormLayout.addRow("Output folder location (optional, will be named 'Keychains'):", self.saveDirSelector)
...
class BrainKeyCreatorExtensionLogic(ScriptedLoadableModuleLogic):
  ...
  def run(savedFilePath, self, leftVol, rightVol, meshScaleFactor, handleShift, enableScreenshots=0):
    ...
    print(leftData)
    print(rightData)
    leftBound = min(leftData[0], rightData[0])
    rightBound = max(leftData[1], rightData[1])
    ...
    holesFilter.SetForegroundValue(255)
    holesFilter.SetFullyConnected(True)
